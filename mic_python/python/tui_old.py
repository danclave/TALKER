...
def _normalize_key(key, data=None):
    ...
    if name in mapping:
        return mapping[name]
    if name == " ":  # raw space char
        return "space"
    if len(name) == 1:
        return name
    return None  # unrecognized key: callers must skip it (not EOF!)
...
def get_key(key_source):
    """Next *recognized* key press; skips unrecognized keys.

    Returns None ONLY when the key stream has truly ended (EOF), never for
    unrecognized keys - callers treat None as quit.
    """
    try:
...
def _pick_list(console, key_source, title, items, footer=None):
    ...
    while True:
        ...
        foot = footer or _footer(("Up/Dn", "move"), ("Enter", "select"), ("Esc", "back"), ("q", "quit"))
        _clear(console)
        console.print(panel)
...
def _language_picker(console, key_source, settings):
    ...
    while True:
        ...
        _clear(console)
        console.print(panel)
        console.print(_footer(("type", "filter"), ("Up/Dn", "move"), ("Enter", "select"),
...
def _whisper_picker(console, key_source, settings):
    ...
    result = _pick_list(console, key_source, "Whisper model size", rows,
                        footer=_footer(("Up/Dn", "move"), ("Enter", "select"), ("Esc", "back")))
...
def run_tui(settings, on_test=None, console=None, key_source=None):
    ...
    while True:
        ...
        layout.add_row(Align.center(_footer(("Up/Dn", "move"), ("Enter", "select"),
                                           ("s", "save"), ("q", "quit"))))
