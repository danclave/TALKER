# loading_variants.py
# the TALKER boot-animation library - pure rich render functions, no
# textual dependency. Shared by the real boot screen (tui.py) and the
# gallery (loading_showcase.py).
#
# BOOT_VARIANTS: the curated set the real app randomly picks from at
# every launch (user-approved picks from two showcase rounds).
import math
...
# ---------------------------------------------------------------- curated sets
# the 10 user-approved boot animations (showcase rounds 1+2 picks).
# Randomly chosen by LoadingScreen at every launch.
BOOT_VARIANTS = [
    VClassicFill(),     # round 1 pick
    VRadialBloom(),     # round 1 pick (bar-free)
    VGeigerTicks(),     # round 1 pick
    VMatrixRain(),      # round 1 pick (bar-free)
    VSinePulse(),       # round 1 pick
    VCRTPowerOn(),      # round 1 pick - "the coolest one"
    VBootLog(),         # round 1 pick
    VOscilloscope(),    # round 2 pick
    VCipherDecrypt(),   # round 2 pick - "very nice one"
    VZoneDawn(),        # round 2 pick
]

# everything, for the gallery
ALL_VARIANTS = [
    VClassicFill(), VRadialBloom(), VGeigerTicks(), VMatrixRain(),
    VSinePulse(), VCRTPowerOn(), VBootLog(), VPowerFault(), VDustWind(),
    VOscilloscope(), VCipherDecrypt(), VZoneDawn(), VThermalSweep(),
    VCircuitCurrent(), VBreathingPDA(), VTVStatic(), VFlipCountdown(),
]
