...
# Lists hold alternative models per language; the FIRST entry is the default
# (latest version). Sizes >= VOSK_BIG_MB are large: slow and resource hungry.
...
VOSK_MODELS = {
    ...
    # ru: 0.4 is the latest (default); 0.22 kept as alternative - it is more
    # accurate on some common words (benchmarked Aug 2026, 0.4: друг -> вдруг)
    "ru":    [("vosk-model-small-ru-0.4", 39), ("vosk-model-small-ru-0.22", 44)],
...
def vosk_model_options(code):
    """Return the list of (model_name, size_mb) for a language, default first."""
    return list(VOSK_MODELS.get((code or "").lower(), []))


def vosk_model_by_name(code, name):
    """Return (model_name, size_mb) for a specific model of a language, or None."""
    for model_name, size_mb in vosk_model_options(code):
        if model_name == name:
            return (model_name, size_mb)
    return None


def vosk_model_info(code):
    """Return the default (latest) (model_name, size_mb) for a language, or None."""
...
def whisper_supported(code):
    """True if whisper supports this language (accent codes count, e.g. en-gb)."""
    global _whisper_codes
    if _whisper_codes is None:
        try:
            from faster_whisper.tokenizer import _LANGUAGE_CODES
            _whisper_codes = set(_LANGUAGE_CODES)
        except Exception:
            _whisper_codes = set()  # import failed: use static fallback below
    base = (code or "").split("-", 1)[0].lower()
    if _whisper_codes:
        return base in _whisper_codes
    return base not in ("eo", "ky")  # static fallback: only known gaps
